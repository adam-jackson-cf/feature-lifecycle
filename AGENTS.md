...
- App code lives in `dashboard/` (Next.js 16 + TS): `app/` routes, `lib/` (db, repositories, services, utils, types), `components/` (to be expanded), `tests/` (unit + integration stub), `public/` assets.
...
- Install deps: `cd dashboard && bun install`.
- Dev server: `bun run dev` (http://localhost:3000).
- Unit tests: `bun run test:unit` (Vitest, mock data).
- Integration tests: `bun run test:integration` (hits public Kafka Jira/GitHub; requires network).
- All tests: `bun run test:run`.
- Lint: `bun run lint`; TypeScript check: `bun run typecheck`.
...
- Style: Ultracite (Biome-based) - see `biome.jsonc`; run lint before commits.
...
- Before PR: run `bun run lint`, `bun run typecheck`, `bun run test:unit` (and integration when touching import/network code).
